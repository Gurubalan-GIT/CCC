        ...
        for(int i = 0; i < n; i ++)
        {
            if(prod[i][0] == sym)
            {
                for(int j = 1; prod[i][j] != '\0'; j ++)
                {
                    ...
                    {
                       cout<<"Leading("<<sym<<") = Leading("<<prod[i][j+1]<<")"<<endl;
                       Leading(prod[i][j+1], prod, n);
                    }
                   else if(prod[i][j-1] == '>' || prod[i][j-1] == '|'){
                       cout<<"Leading("<<sym<<") = Leading("<<prod[i][j]<<")"<<endl;
...
char trail[20];
bool notin(char sym,char str[])
{
    for(int i = 0; str[i] != '\0';i++)
        if(str[i] == sym)return 0;
    return 1;
}
char * Trailing(char sym, char prod[][20], int n)
{
    int k = strlen(trail);
    if(!isalpha(sym) && notin(sym,trail)){trail[k] = sym; trail[k+1] = '\0'; cout<<"Trail becomes : {"<<trail<<"}"<<endl;}
    else if(islower(sym) && notin(sym,trail)){trail[k] = sym; trail[k+1] = '\0'; cout<<"Trail becomes : {"<<trail<<"}"<<endl;}
    else
    {
        for(int i = 0; i < n; i ++)
        {
            if(prod[i][0] == sym)
            {
                for(int j = strlen(prod[i]) - 1; j >=0; j--)
                {
                    ...
                    {
                        cout<<"Trailing("<<sym<<") = Trailing("<<prod[i][j-1]<<")"<<endl;
                        Trailing(prod[i][j-1],prod,n);
                    }
                    if(prod[i][j+1] == '\0' || prod[i][j+1] == '|')
                    {
                        cout<<"Trailing("<<sym<<") = Trailing("<<prod[i][j]<<")"<<endl;
